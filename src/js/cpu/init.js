function initOpcodes (cpu, memory) {
  ...
  // Branch Instructions
  initOp(cpu, [[0x90, 2, memory.relative]], cpu.bcc,
         "Branch on Carry Clear", false);
  initOp(cpu, [[0xb0, 2, memory.relative]], cpu.bcs,
         "Branch on Carry Set", false);
  initOp(cpu, [[0xf0, 2, memory.relative]], cpu.beq,
         "Branch on Equal", false);
  initOp(cpu, [[0x30, 2, memory.relative]], cpu.bmi,
         "Branch on Result Minus", false);
  initOp(cpu, [[0xd0, 2, memory.relative]], cpu.bne,
         "Branch on Not Equal", false);
  initOp(cpu, [[0x10, 2, memory.relative]], cpu.bpl,
         "Branch on Result Plus", false);
  initOp(cpu, [[0x50, 2, memory.relative]], cpu.bvc,
         "Branch on Overflow Clear", false);
  initOp(cpu, [[0x70, 2, memory.relative]], cpu.bvs,
  ...
  initOp(cpu, [[0x20, 3, memory.absolute, true]], cpu.jsr,
         "Jump To Subroutine", false);

  // Clear Flag Instructions
  initOp(cpu, [[0x18, 1, null]], cpu.clc, "Clear Carry Flag");
  initOp(cpu, [[0xd8, 1, null]], cpu.cld, "Clear Decimal Flag");
  initOp(cpu, [[0x58, 1, null]], cpu.cli, "Clear Interrupt Flag");
  initOp(cpu, [[0xb8, 1, null]], cpu.clv, "Clear Overflow Flag");

  // Set Flag Instructions
  initOp(cpu, [[0x38, 1, null]], cpu.sec, "Set Carry Flag");
  initOp(cpu, [[0xf8, 1, null]], cpu.sed, "Set Decimal Flag");
  initOp(cpu, [[0x78, 1, null]], cpu.sei, "Set Interrupt Flag");

  // Stack Instructions
  initOp(cpu, [[0x48, 1, null]], cpu.pha, "Push Accumulator to Stack");
  initOp(cpu, [[0x08, 1, null]], cpu.php, "Push Status to Stack");
  initOp(cpu, [[0x68, 1, null]], cpu.pla, "Pull Accumulator from Stack");
  initOp(cpu, [[0x28, 1, null]], cpu.plp, "Pull Status from Stack");

  // Transfer Instructions
  initOp(cpu, [[0xaa, 1, null]], cpu.tax, "Transfer Accumulator to X");
  initOp(cpu, [[0xa8, 1, null]], cpu.tay, "Transfer Accumulator to Y");
  initOp(cpu, [[0xba, 1, null]], cpu.tsx, "Transfer Stack Pointer to X");
  initOp(cpu, [[0x8a, 1, null]], cpu.txa, "Transfer X to Accumulator");
  initOp(cpu, [[0x9a, 1, null]], cpu.txs, "Transfer X to Stack Pointer");
  initOp(cpu, [[0x98, 1, null]], cpu.tya, "Transfer Y to Accumulator");

  // Misc Implied Instructions
  initOp(cpu, [[0x00, 1, null]], cpu.brk, "Force Break");
  initOp(cpu, [[0xca, 1, null]], cpu.dex, "Decrement X");
  initOp(cpu, [[0x88, 1, null]], cpu.dey, "Decrement Y");
  initOp(cpu, [[0xe8, 1, null]], cpu.inx, "Increment X");
  initOp(cpu, [[0xc8, 1, null]], cpu.iny, "Increment Y");
  initOp(cpu, [[0xea, 1, null]], cpu.nop, "No Operation");
  initOp(cpu, [[0x40, 1, null]], cpu.rti, "Return from Interrupt");
  initOp(cpu, [[0x60, 1, null]], cpu.rts, "Return from Subroutine");
}
...
function buildOp (cpu, version, method, trackPC) {
  let [bytes, addrMode, raw] = version;
